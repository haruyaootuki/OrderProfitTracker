...
class TestAdminUsersPage:
    ...
    def test_admin_user_access(self, client, admin_user):
        ...
        client.post(url_for('main.login'), data={'username': 'admin', 'password': 'password', 'csrf_token': csrf_token}, follow_redirects=True)
    ...
    def test_non_admin_access_redirect(self, client, non_admin_user):
        csrf_token = self._get_csrf_token(client)
        assert csrf_token is not None
        client.post(url_for('main.login'), data={'username': 'user', 'password': 'password', 'csrf_token': csrf_token}, follow_redirects=True)
        response = client.get(url_for('main.admin_users'))
        assert response.status_code == 302
        assert url_for('main.index') in response.headers['Location']
    ...
    def test_database_connection_failure(self, client, admin_user, monkeypatch, app):
        csrf_token = self._get_csrf_token(client)
        assert csrf_token is not None
        login_response = client.post(url_for('main.login'), data={'username': 'admin', 'password': 'password', 'csrf_token': csrf_token}, follow_redirects=True)
