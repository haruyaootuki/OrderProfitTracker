...
class TestApiCreateOrder:
    ...
    def test_order_creation_rate_limit(self, client, authenticated_user):
        ...
        for _ in range(30):
            response = client.post(url_for('main.api_create_order'), data=data, content_type='application/x-www-form-urlencoded')
            assert response.status_code == 201 # Now that CSRF is handled, these should be 201
        response = client.post(url_for('main.api_create_order'), data=data, content_type='application/x-www-form-urlencoded') # Last request should hit rate limit
    ...
    def test_create_order_invalid_data(self, client, authenticated_user):
        ...
        response = client.post(url_for('main.api_create_order'), data={'csrf_token': csrf_token}, content_type='application/x-www-form-urlencoded') # Send minimal data with CSRF to trigger validation
    ...
    def test_create_order_unauthenticated(self, client, monkeypatch, app):
        ...
        response = client.post(url_for('main.api_create_order'), data={}, content_type='application/x-www-form-urlencoded') # Data can be empty if it's blocked by login_required
